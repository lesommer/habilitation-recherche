...
La représentation des intéractions d'échelles est un enjeu crucial pour les modèles de circulation océanique; cette question concentre donc à ce titre une part importante des efforts de la communauté des modélisateurs. En pratique, cette question peut se poser de deux manières, selon que l'on souhaite représenter des interactions entre des échelles a priori explicitement résolues par le modèle ou non. La bonne représentation des interactions entre échelles explicitement résolues passe généralement par un travail sur les méthodes de discretisation dans le but de garantir une bonne *précision* numérique, notamment pour les termes *non-lineaires* des équations primitives. La représentation des interactions impliquant des échelles non-explicitement résolues par le modèle pose un problème classique, dit de fermeture sous-maille; un problème abordé en formulant des *paramétrisations physiques* qui visent à modéliser de manière simplifiée le résultat net des interactions sur les échelles résolues et non le détail phénoménologique des processus en jeu.  

Bien que le terme de *paramétrisation physique* soit un terme de jargon assez spécifique au champ de la modélisation des fluides environnementaux, le problème de la fermeture sous-maille est un problème générique en modélisation numérique des systèmes non-linéaires, notamment en mécanique des fluides. Comme le rappellent Griffies et Treguier (2013) [@doi:10.1016/B978-0-12-391851-2.00020-9], les écoulements océaniques sont turbulents jusqu'aux échelles de $10^{-3}$m et $1$s de sorte que simuler explicitement toutes les échelles de la circulation océanique demanderait environ $10^{27}$ points de grilles, ce qui dépasse de très loin la capacité des calculateurs. Les modèles de circulation océanique ont donc pour objet de décrire la circulation océanique *moyenne* (au sens d'une moyenne spatio-temporelle, ou au sens d'une moyenne d'ensemble sur les réalisations possibles de la circulation étant données les informations dont on dispose à une certaine échelle). Toutefois comme les équations primitives sont essentiellement *non-linéaires*, les équations qui régissent l'évolution de cette circulation *moyenne* ne sont pas fermées (voir les équations (@eq:fermeture) ci-dessous) : il nous faut donc formuler des hypothèses sur l'effet des échelles non-résolues sur les échelles résolues pour obtenir un système qui puisse être intégré numériquement.    

$$
\partial_t \overline{\mathbf{u}} 
+ \mathcal{L}_u \,\overline{\mathbf{u}} 
+ \mathcal{N}_u \, (\overline{\mathbf{u}}) = \mathcal{N}_u \, (\overline{\mathbf{u}}) - \overline{\mathcal{N}_u \, (\mathbf{u})}
\; 
\qquad
\partial_t \overline{b} 
+ \mathcal{L}_b \,\overline{\mathbf{b}} 
+ \mathcal{N}_b \, \overline{\mathbf{b}} = \mathcal{N}_b \, (\overline{\mathbf{b}}) - \overline{\mathcal{N}_b \, (\mathbf{b})}
\label{eq:averaging}
$$ {#eq:fermeture}

où $\mathcal{N}_b$ blabla. 

Il existe deux grand cadres conceptuels pour concevoir des fermetures sous-mailles en mécanique des fluides et les modèles océaniques piochent alternativement dans l'un et l'autre de ces cadres selon les besoins. LES [@url:https://www.springer.com/gp/book/9783540263449] ou RANS [@doi:10.1002/9781119176817.ecm2061]. micro-ectats/macro-etats. hypothèse ergodique : on remplace pare une moyenne temporelle. souvent supposé deterministe mais c'est une restriction.  dans le cas oceanique : En pratique fermeture pour equation de transport et pour equation de quantité de  mouvement. on distingue classiquement vertical et horizontal. car grille tres anisotrope.  semble simple et pourtant  pas de transferts direct des approches RANS/LES vers les modèles de circulation océaniques [@doi:10.1029/177GM19]. 
